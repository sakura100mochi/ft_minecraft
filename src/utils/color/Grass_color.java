...
public final class Grass_color implements IColor {
	...
	public Grass_color(Data data) throws Exception {
		if (data == null || data.textureManager == null || data.worldgen == null || data.parser == null) {
			throw new IllegalArgumentException("utils.color.Grass_color | Invalid argument");
		}
	
		this.data = data;
		this.colorMap = data.textureManager.getTextureInfo("colormap/grass.png");
		if (this.colorMap == null || this.colorMap.getPixelBuffer() == null) {
			throw new IllegalStateException("utils.color.Grass_color | Failed to load grass color map");
		}
	}
	...
	public byte[] getColor(int x, int y, int z) throws Exception {
		...
		if (effects != null && effects.has("grass_color") == true) {
			return HexColor.convertToByte(effects.getString("grass_color"));
		}
