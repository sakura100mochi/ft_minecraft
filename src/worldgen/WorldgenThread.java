...
public final class WorldgenThread extends Thread {
	...
	private void update() throws Exception {
		...
		if (chunk != null) {
			...
			ByteBuffer terrain = this.terrainGenerator.generateBuffer(chunk_x, chunk_z, generateRegistries(chunk_x, chunk_z));
			this.data.chunkManager.addUpdateEvent(chunk, "terrain", terrain);
			ByteBuffer water = this.waterGenerator.generateBuffer(chunk_x, chunk_z, generateRegistries(chunk_x, chunk_z));
	...
	private void cleanup() {
		...
		if (registriesCache.size() > 64) {
			Iterator<Long> it = this.registriesCache.keySet().iterator();
			while (it.hasNext()) {
				long key = it.next();
	...
	public int[][] getWORLD_SURFACE_WG(int chunk_x, int chunk_z, int[] registries) throws Exception {
		long key = Position2D.toLong(chunk_x, chunk_z);
		return this.data.worldgen.overworld.height_map.generateWORLD_SURFACE_WG(chunk_x, chunk_z, registries, WORLD_SURFACE_Cache.computeIfAbsent(key, value -> new int[16][16]));
	}

	public int[][] getWORLD_SURFACE(int chunk_x, int chunk_z, int[] registries) throws Exception {
		long key = Position2D.toLong(chunk_x, chunk_z);
		return this.data.worldgen.overworld.height_map.generateWORLD_SURFACE(chunk_x, chunk_z, registries, WORLD_SURFACE_Cache.computeIfAbsent(key, value -> new int[16][16]));
	}

	public int[][] getOCEAN_FLOOR_WG(int chunk_x, int chunk_z, int[] registries) throws Exception {
		long key = Position2D.toLong(chunk_x, chunk_z);
		return this.data.worldgen.overworld.height_map.generateOCEAN_FLOOR_WG(chunk_x, chunk_z, registries, OCEAN_FLOOR_Cache.computeIfAbsent(key, value -> new int[16][16]));
	}

	public int[][] getOCEAN_FLOOR(int chunk_x, int chunk_z, int[] registries) throws Exception {
		long key = Position2D.toLong(chunk_x, chunk_z);
		return this.data.worldgen.overworld.height_map.generateOCEAN_FLOOR(chunk_x, chunk_z, registries, OCEAN_FLOOR_Cache.computeIfAbsent(key, value -> new int[16][16]));
	}

	public int[][] getMOTION_BLOCKING(int chunk_x, int chunk_z, int[] registries) throws Exception {
		long key = Position2D.toLong(chunk_x, chunk_z);
		return this.data.worldgen.overworld.height_map.generateMOTION_BLOCKING(chunk_x, chunk_z, registries, MOTION_BLOCKING_Cache.computeIfAbsent(key, value -> new int[16][16]));
	}

	public int[][] getMOTION_BLOCKING_NO_LEAVES(int chunk_x, int chunk_z, int[] registries) throws Exception {
		long key = Position2D.toLong(chunk_x, chunk_z);
		return this.data.worldgen.overworld.height_map.generateMOTION_BLOCKING_NO_LEAVES(chunk_x, chunk_z, registries, MOTION_BLOCKING_NO_LEAVES_Cache.computeIfAbsent(key, value -> new int[16][16]));
	}
	...
	public int[][] getHeightMap(int chunk_x, int chunk_z, String type) throws Exception {
		...
		switch (type) {
			case "WORLD_SURFACE_WG":
				return getWORLD_SURFACE_WG(chunk_x, chunk_z, registries);
			case "WORLD_SURFACE":
				return getWORLD_SURFACE(chunk_x, chunk_z, registries);
			case "OCEAN_FLOOR_WG":
				return getOCEAN_FLOOR_WG(chunk_x, chunk_z, registries);
			case "OCEAN_FLOOR":
			...
			default:
				throw new RuntimeException("Unsupported height map type: " + type);
		}
	}
	...
	private int[] generateRegistries(int chunk_x, int chunk_z) throws Exception {
		...
		for (int x = chunk_x - 1; x <= chunk_x + 1; x++) {
			for (int z = chunk_z - 1; z <= chunk_z + 1; z++) {
				...
				this.data.worldgen.overworld.features.generateFeatures(x, z, current_registries);
			}
		}
		long key = Position2D.toLong(chunk_x, chunk_z);
	...
	public int[] getRegistries(int chunk_x, int chunk_z) {
		long key = Position2D.toLong(chunk_x, chunk_z);
		return this.registriesCache.computeIfAbsent(key, value -> new int[SystemSettings.CHUNK_SIZE * SystemSettings.CHUNK_SIZE * this.terrainHeight + 1]);
	}
	...
	public int[] getTransparency(int chunk_x, int chunk_z) throws Exception {
		...
		return this.transparencyCache.computeIfAbsent(key, k -> {
			int[] result = new int[SystemSettings.CHUNK_SIZE * SystemSettings.CHUNK_SIZE * this.terrainHeight + 1];
			int airId = Registry.getId("minecraft:air");
			Arrays.fill(result, 1, result.length, airId);
			return result;
		});
	}
